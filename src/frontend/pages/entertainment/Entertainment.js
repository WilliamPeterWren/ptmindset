...
import { entertainmentFolderId } from "../../../api/entertainmentFolderId";
import { entertainmentApiKey } from "../../../api/apiKey";

function Entertainment() {
  useEffect(() => {
    document.title = "Entertainment";
    return () => (document.title = "Peter");
  }, []);
    ...
    (async () => {
      try {
        ...
        const results = await Promise.all(
          entertainmentFolderId.map((id) =>
            getDriveUrls(id, entertainmentApiKey)
          )
        );
  ...
  const next = () => setCurrentIndex((i) => (i + 1) % shuffled.length);
  const prev = () =>
    setCurrentIndex((i) => (i === 0 ? shuffled.length - 1 : i - 1));
  ...
  return (
    <div className="mx-4 px-4 pt-4 bg-customLightDark min-h-screen">
      <h1 className="font-semibold text-lg">
        Giải trí điiii – Kho: {shuffled.length} clips
      </h1>
