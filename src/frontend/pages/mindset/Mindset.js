...
import { mindsetFolderId } from "../../../api/mindsetFolderId";
import { mindsetApiKey } from "../../../api/apiKey";

function Mindset() {
  useEffect(() => {
    document.title = "Mindset";
    ...
    (async () => {
      try {
        ...
        const results = await Promise.all(
          mindsetFolderId.map((id) => getDriveUrls(id, mindsetApiKey))
        );
  ...
  return (
    <div className="mx-4 px-4 pt-4 bg-customLightDark min-h-screen">
      <h1 className="font-semibold text-lg">
        Học thêm gì đó - Kho: {shuffled.length} clips
      </h1>
