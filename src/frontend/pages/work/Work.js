...
import { workFolderId } from "../../../api/mindsetFolderId";
    ...
    (async () => {
      try {
        ...
        const results = await Promise.all(
          workFolderId.map((id) => getDriveUrls(id, mindsetApiKey))
        );
  ...
  return (
    <div className="mx-4 px-4 pt-4 bg-customLightDark min-h-screen">
      <h1 className="font-semibold text-lg">
        Sống và làm việc có trách nhiệm - Kho: {shuffled.length} clips
      </h1>
